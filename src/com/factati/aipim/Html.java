...
public class Html {
	...
	public void generateFeaturesHtml(List<Feature> listFeatures) throws IOException{
		...
		for(Feature feature : listFeatures){
			...
			//for(Feature t : listFeatures){
				output.write("				<li><a href=\"" + feature.getFilename() + ".html\">" +
					(j+1) + ". " + feature.getFeatureName() + "</a></li>");
				output.newLine();
				//if (i == j){
					output.write("					<li>");
					output.newLine();
					output.write("						<ul class=\"list-unstyled features-link\">");
					for(ScenarioAipim scenario : feature.getScenario()){
			...
			for(ScenarioAipim scenario : feature.getScenario()){
				...
				output.newLine();
				output.write("    		<h3 class=\"panel-title\">" + (j+1) + "." + (i+1) + ". " +
						scenario.getName() + 
				...
				if((scenario.getScreenshot() != null) || (!scenario.getScreenshot().equals(""))){
					...
					output.write("  			<a href=\"../screenshots/" + feature.getFilename() + 
							"/" + scenario.getScreenshot() + "\"><img src=\"../screenshots/" + 
							feature.getFilename() + "/" + scenario.getScreenshot() + "\" /></a>");
					output.newLine();
					output.write("  		</div>");
					output.newLine();
				}
